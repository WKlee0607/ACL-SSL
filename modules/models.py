...
class ACL(nn.Module):
    ...
    def forward(self, image: torch.Tensor, embedding: torch.Tensor, resolution: int = 224) -> dict:
        ...
        if self.training:
            ...
            # input 정의
            seg_logit = self.forward_module(image, embedding, resolution)
            spatial_feat = self.av_grounder.get_pixels(image) # v^D: [B, c, h, w] # [B, 512, 22, 22]
            
            # forward_ideas
            refined_heatmap = self.forward_ideas(seg_logit, spatial_feat, embedding, resolution)
    ...
    ########## implements ideas 
    def forward_ideas(self, seg_logit, spatial_feat, embedding, resolution: int = 224 ):
        '''
        Return refined heatmap reflecting visual feature similarity 

        Args :
            seg_logit : [batch, channel, height, width], same size with heatmap.
            spatial_feat : [batch, channel, embed_size, embed_size] -> [batch, 512, 22, 22]
            embedding : [1, channel] -> [B, 512]?
            resolution (int): Resolution of the output tensor.
        '''
        # 1. audio embedding과 spatial feature와 atten score 계산
        B, _, _, _ = spatial_feat.shape
        embed_size = spatial_feat.size(dim=-1) # 22

        embedding = embedding.unsqueeze(0).repeat(B, 1, 1)
        atten_score = embedding @ spatial_feat.reshape((B, -1, int(embed_size**2))) # [B, 1, embed_size**2] - [B, 1, 484]

        # 2. reshaped & interpol
        atten_reshaped = torch.reshape(atten_score, (-1, 1, int(embed_size), int(embed_size))) # [batch , 1, embed_size, embed_size]
        atten_interpolated = F.interpolate(atten_reshaped, (resolution, resolution), mode='bicubic') # [batch, 1, resolution, resolution]
        
        # 3. norm same interval with seg_logit
        i_min, i_max = torch.amin(atten_interpolated, dim=(1, 2, 3), keepdim=True), torch.amax(atten_interpolated, dim=(1, 2, 3), keepdim=True) # [B, 1, 1, 1]         
        a,b = torch.amin(seg_logit, dim=(1, 2, 3), keepdim=True), torch.amax(seg_logit, dim=(1, 2, 3), keepdim=True) # [B, 1, 1, 1]
        atten_norm = a + (b-a)*(atten_interpolated - i_min) / (i_max - i_min)   # similarity [a, b] 구간으로 norm # [B, 1, resolution, resolution]

        # test
        refined_seg_logit = (seg_logit + atten_norm) / 2
        refined_heatmap = self.masker_i(refined_seg_logit, infer=True)
